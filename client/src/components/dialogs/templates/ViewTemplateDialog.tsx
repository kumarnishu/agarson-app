            ...
            >
                ...
                <DialogContent>
                    {template.message || template.media ? <Stack sx={{ bgcolor: 'black', maxWidth: '350px', p: 2 }}>
                        {template.message && <Typography sx={{ p: 1, m: 1, bgcolor: 'lightgreen', whiteSpace: 'pre-line', border: 1, borderColor: 'darkgreen', borderRadius: 1 }}>{template.message.replaceAll("\\n", "\n").replaceAll("\\t", "\t")}</Typography>}
                        {template.media && <Stack sx={{ bgcolor: 'lightgreen', m: 1, p: 1, wordBreak: 'break-all', border: 5, borderColor: 'darkgreen', borderRadius: 2 }}>
                            ...
                            {template.media && <Typography sx={{ py: 1 }}>{template.caption?.replaceAll("\\n", "\n").replaceAll("\\t", "\t")}</Typography>}
                        </Stack>}
                    </Stack> : null}
                </DialogContent>
