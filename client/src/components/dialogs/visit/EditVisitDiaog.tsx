...
import EditVisitForm from '../../forms/visit/EditVisitForm';

function EditVisitInDialog({ visit }: { visit: IVisitReport }) {
    const { choice, setChoice } = useContext(ChoiceContext)
    return (
        <>
            <Dialog fullScreen={Boolean(window.screen.width < 500)} open={choice === VisitChoiceActions.edit_visit ? true : false}
            > <IconButton style={{ display: 'inline-block', position: 'absolute', right: '0px' }} color="error" onClick={() => setChoice({ type: VisitChoiceActions.close_visit })}>
                ...
                <DialogContent>
                    {visit && <EditVisitForm visit={visit} />}
                </DialogContent>
            </Dialog>
        </>
    )
...
export default EditVisitInDialog
