...
export const BulkContactUpdateFromExcel = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.file) {
        ...
        let workbook_response: { name: string, designation: string, mobile: string }[] = xlsx.utils.sheet_to_json(
            workbook.Sheets[workbook_sheet[0]]
        );
        console.log(workbook_response)
        let newContacts: { name: string, designation: string, mobile: string }[] = []
        workbook_response.forEach(async (lead) => {
            let mobile: string | null = lead.mobile
            let name: string | null = lead.name
            let designation: string | null = lead.designation
            console.log(mobile, name)
            newContacts.push({ name: name, designation: designation, mobile: "91" + mobile + "@c.us" })
        })
        console.log(newContacts)
        newContacts.forEach(async (ct) => {
            let contact = await Contact.findOne({ mobile: ct.mobile })
            if (!contact)
                await new Contact({ name: ct.name, designation: ct.designation, mobile: ct.mobile, created_by: req.user, updated_by: req.user }).save()
        })
    }
