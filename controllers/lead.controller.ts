...
import { ILead, TLeadBody, ILeadTemplate, TLeadUpdatableFieldBody, TReferredPartyBody, Asset, IReferredParty } from "../types"
            ...
            $and: [
                ...
                },
                {
                    $or: [
                        ...
                        { lead_owners_username: { $regex: key[1], $options: 'i' } },
                    ]
                }
            ]
            ,
            ...
            $and: [
                ...
                },
                {
                    $or: [
                        ...
                        { lead_owners_username: { $regex: key[2], $options: 'i' } },
                    ]
                }
            ]
            ,
            ...
            $and: [
                ...
                },
                {
                    $or: [
                        ...
                        { lead_owners_username: { $regex: key[3], $options: 'i' } },
                    ]
                }
            ]
            ,
...
export const GetLeadsReport = async (req: Request, res: Response, next: NextFunction) => {
    let leads = await Lead.find({ is_customer: false })
    ...
    users.forEach((user) => {
        let tmpleads = leads.filter((lead) => {
            let owners = lead.lead_owners.filter((owner) => {
        ...
        if (tmpleads && tmpleads.length > 0)
            response.push({
                username: user.username,
                leads: tmpleads.length
            })
    })
